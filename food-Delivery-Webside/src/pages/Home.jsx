  ...
  return (
    <div className=' bg-slate-200 w-full min-h-screen'>
       ...
       {!input? <div className='flex flex-wrap justify-center items-center gap-5 w-[100%] '>
        {Categories.map((item)=>{
         return(
         <div className='w-[140px] h-[150px] bg-white flex flex-col items-start gap-5 p-5 justify-start text-[20px] font-bold text-gray-600
         rounded-lg shadow-xl hover:bg-green-300 cursor-pointer transition-all duration-200' onClick={()=>filter(item.name)}>
          {item.icon}
          {item.name}
          </div>
         )
      ...
      <div className='w-full flex flex-wrap gap-5 px-5 justify-center items-center pt-8 pb-8'>
        ...
        )):<div  className='text-center text-2xl text-green-500 font-semibold pt-7'>No dish Found</div>}
      ...
      shadow-xl p-6  transition-all duration-500 flex flex-col items-center overflow-auto ${showCart?"translate-x-0":"translate-x-full"}`}>
        ...
        <RxCross2 className=' h-[30px] w-[30px] text-green-400 text-[23px] font-semibold cursor-pointer hover:text-gray-600' onClick={()=>
          setShowCart(false)
        }/>
        </header>
        ...
        <div>
          <div className='w-full flex justify-start items-center gap-70 p-8'>
       ...
       text-white  hover:bg-green-400 transition-all cursor-pointer' onClick={()=>{
        toast.success("order placed ")
          ...
          <div className='text-center text-2xl text-green-500 font-semibold pt-7'>
            Empty cart
            </div>}
