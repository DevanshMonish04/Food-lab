...
function Nav() {
  const { input, setInput, setCat ,showCart,setShowCart } = useContext(dataContext);
  ...
  return (
    <div className='w-full h-[100px] flex justify-between items-center px-5 md:px-8'>
      ...
      <form className='w-[45%] h-[60px] bg-white flex items-center
        px-5 gap-5 rounded-md shadow-md md:w-[70%]'
        onSubmit={(e)=>e.preventDefault()}>
        <FaSearch className='text-green-600 h-[20px] w-[20px]' />
        <input
          type='text'
      ...
      <div className='w-[60px] h-[60px] bg-white flex justify-center
        items-center rounded-md shadow-xl relative cursor-pointer' onClick={()=>setShowCart(true)}>
        <span className='absolute top-0 right-3 text-green-600 font-bold text-[18px]'>{items.length}</span>
